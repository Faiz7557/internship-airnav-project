        ...
        @if($growth !== null || $subValue)
            <div class="mt-4 flex items-center gap-2">
                @if($growth !== null)
                    ...
                    @endphp
                    <span {{ $idContainer ? 'id=' . $idContainer : '' }} class="bg-{{ $color }}-50/80 backdrop-blur text-{{ $color }}-600 text-sm px-3 py-1 rounded-full font-bold shadow-sm border border-{{ $color }}-100 flex items-center gap-1 group-hover:bg-{{ $color }}-100 transition-colors">
                        <span {{ $idIcon ? 'id=' . $idIcon : '' }}>{!! $isPositive ? '▲' : '▼' !!}</span>
                        <span {{ $idText ? 'id=' . $idText : '' }}>{{ abs($growth) }}%</span>
                    </span>
                    @if($growthText)
                        <span {{ $idVs ? 'id=' . $idVs : '' }} class="text-xs text-slate-400 font-medium">{{ $growthText }}</span>
                    @endif
                @elseif($subValue)
                     <span class="text-sm font-semibold text-slate-500 bg-slate-100/80 px-3 py-1 rounded-lg">
                        {{ $subValue }}
                     </span>
                @endif
            </div>
